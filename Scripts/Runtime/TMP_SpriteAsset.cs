    ...
    public class TMP_SpriteAsset : TMP_Asset
    {
        ...
        void Awake()
        {
            ...
            if (this.material != null && string.IsNullOrEmpty(m_Version))
                UpgradeSpriteAsset();
            foreach (var sprite in spriteInfoList)
            {
                AdjustSpriteSize(sprite);
            }
        }

        private void AdjustSpriteSize(TMP_Sprite sprite)
        {
            float spriteWidth = sprite.width;
            float spriteHeight = sprite.height;

            sprite.xAdvance = spriteWidth;
            // sprite.yAdvance = spriteHeight;

            sprite.scale = 1.0f;

            sprite.xOffset = sprite.pivot.x - spriteWidth / 2;
            sprite.yOffset = sprite.pivot.y - spriteHeight / 2;
        }
        /// <summary>
